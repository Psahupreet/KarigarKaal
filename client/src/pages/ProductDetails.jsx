...
export default function ProductDetails() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:6000/api/products/${id}`)
      .then((res) => {
        setProduct(res.data);
        setIsLoading(false);
      })
      ...
      <div className="grid md:grid-cols-2 gap-6 sm:gap-8 md:gap-10 lg:gap-12">
        ...
        <div>
          <div className="mb-3 sm:mb-4 rounded-lg sm:rounded-xl overflow-hidden shadow-md sm:shadow-lg">
            <img
              src={`http://localhost:6000/uploads/${product.images[mainImage]}`}
              alt={product.name}
              className="w-full h-64 sm:h-80 md:h-96 object-cover cursor-pointer"
              onClick={() => window.open(`http://localhost:6000/uploads/${product.images[mainImage]}`, '_blank')}
            />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
            {product.images.map((img, index) => (
              ...
              >
                <img
                  src={`http://localhost:6000/uploads/${img}`}
