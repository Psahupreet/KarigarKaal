...
export default function ProductList() {
  ...
  useEffect(() => {
    axios.get("http://82.29.165.206:8080/api/products")
      .then((res) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <div className="text-center mb-8 sm:mb-12">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-3 sm:mb-4">Our Services</h1>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          {[...Array(6)].map((_, index) => (
            <div key={index} className="bg-white rounded-lg sm:rounded-xl shadow-sm sm:shadow-md overflow-hidden animate-pulse">
          ...
          {products.map((product) => (
            ...
            >
              <div className="relative h-40 sm:h-48 overflow-hidden">
                <img
                  src={`http://82.29.165.206:8080/uploads/${product.images[0]}`}
                  alt={product.name}
              ...
              <div className="p-4 sm:p-6">
                <div className="flex justify-between items-start gap-2">
                  <h2 className="text-lg sm:text-xl font-bold text-gray-800 line-clamp-1">{product.name}</h2>
                  <p className="text-base sm:text-lg font-bold text-green-600 whitespace-nowrap">₹{product.price}</p>
                </div>
